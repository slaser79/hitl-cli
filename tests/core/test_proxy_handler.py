...
class TestToolListInterception:
    ...
    async def test_tools_list_only_shows_plaintext_tools(self, mock_load_keys):
        ...
        # Mock backend response with both plaintext and E2EE tools
        mock_backend_tools = {
            "tools": [
        ...
        with patch.object(handler, 'forward_to_backend') as mock_forward:
            mock_forward.return_value = {"result": mock_backend_tools}
    ...
    async def test_request_falls_back_to_plaintext_if_no_device_keys(self, mock_load_keys):
        ...
        with patch.object(handler, 'get_device_public_keys') as mock_get_keys:
            ...
            with patch.object(handler, 'forward_to_backend') as mock_forward:
                ...
                # Verify backend was called with plaintext tool
                mock_forward.assert_called_once()
                forwarded_request = mock_forward.call_args[0][0]
                
                assert forwarded_request["params"]["name"] == "request_human_input"
                assert forwarded_request["params"]["arguments"]["prompt"] == "Test prompt"
...
class TestResponseDecryption:
    ...
    @patch('hitl_cli.crypto.load_agent_keypair')
    @patch('hitl_cli.proxy_handler.ProxyHandler.forward_to_backend')
    async def test_plaintext_response_returned_unchanged(self, mock_forward, mock_load_keys):
    ...
    @patch('hitl_cli.crypto.load_agent_keypair')
    @patch('hitl_cli.proxy_handler.ProxyHandler.forward_to_backend')
    async def test_decryption_error_handling(self, mock_forward, mock_load_keys):
        ...
        # Mock backend returning invalid encrypted data
        mock_forward.return_value = {
            "jsonrpc": "2.0",
            "id": 1,
            "result": "invalid_encrypted_data"
        }
        ...
        # Either return error or fallback to plaintext
        assert "error" in response or response["result"] == "invalid_encrypted_data"
